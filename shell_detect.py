...
class ShellDetector(threading.Thread):
    ...
    _directory = '.'
    _scan_hidden = True
    _report_format = '/result_logz/shelldetect_%d-%m-%Y_%H%M%S.html'
    ...
    def version(self):
        try:
            ...
            _version = 0
        try:
            _server_version = urllib2.urlopen('/conf/version.db').read()
        except ValueError:
            _server_version = 0
        ...
        try:
            _app_server_version = urllib2.urlopen('/conf/app').read()
